...
### 技术指标搭建 Technical Indicators Construction Presentation {-}
...
总的来说，选股策略模型的程序被设定为每只进入模型的股票执行所有指标/因子的计算并生成交易信号。然而，为了不占用报告（*量化模型搭建说明*）的大部分空间，所有输出（计算结果、数学和统计图表/表格以及图像/可视化）都已被抑制。（一旦某条输出被抑制，将提供一个<span style="color:red;font-weight:700;font-size:18px">红色警告</span>）。在整个*量化模型搭建说明*的每个模型步骤中，`pdf版本`仅显示**苹果公司 Tesla, Inc.** (**AAPL.US**)的输出结果；`html版本`*量化模型搭建说明*中的图像/可视化也适用类似的规则。
...
```{r preb-98, message=FALSE, warning=FALSE, include=FALSE}
...
```{r preb-99, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r preb-1, message=FALSE, warning=FALSE, include=FALSE}
...
```{r preb-2, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-3, echo=TRUE, message=FALSE, warning=FALSE}
# Alternative way: 
...
```{r preb-4, echo=TRUE, message=FALSE, warning=FALSE}
sma_value_result <- 
  mega7data_modified %>%
...
```{r preb-5, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-6, echo=FALSE, message=FALSE, warning=FALSE}
# html output
# Display SMA values
...
```{r preb-7, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-8, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-9, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-10, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-11, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-12, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-13, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r preb-14, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r preb-15, echo=TRUE, fig.pos='H', message=FALSE, warning=FALSE}
...
```{r preb-16, echo=TRUE, message=FALSE, warning=FALSE}
# html output
...
```{r preb-17, echo=TRUE, message=FALSE, warning=FALSE}
# pdf output
...
```{r preb-18, echo=TRUE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
# pdf output
# SMA Candlestick+Volume Plot for AAPL with trading signals
...
```{r preb-19, echo=TRUE, message=FALSE, warning=FALSE}
# html output
...
```{r preb-20, echo=TRUE, fig.height=7, fig.width=12, message=FALSE, warning=FALSE}
# html output
# plotly interactive plot
...
```{r preb-21, echo=TRUE, message=FALSE, warning=FALSE}
# html output
# plotly interactive plot
